...
function setupAutoUpdater() {
  try {
    ...
    ipcMain.on('update:install', () => {
      forceQuit = true;
      // すべてのBrowserViewを破棄してからquitAndInstall
      for (const [, v] of webViews) {
        try { mainWindow.removeBrowserView(v); v.webContents.destroy(); } catch(e) {}
      }
      webViews.clear();
...
ipcMain.on('sb:open', () => {
  ...
  // サイドバー展開中はアクティブなwebViewを画面外に退避してUIのイベントを確実に受け取る
  if (activeTabId) {
    const v = webViews.get(activeTabId);
...
ipcMain.on('sb:close', () => {
  ...
  bringUIToFront();
  mainWindow.setWindowButtonVisibility(false);
  // 少し遅らせてからwebViewを元の位置に戻す（contextmenuが表示される前にwebViewが被さるのを防ぐ）
  setTimeout(() => {
    if (activeTabId) {
      const v = webViews.get(activeTabId);
      if (v) layoutWebView(v);
