const { app, BrowserWindow, BrowserView, ipcMain, Menu, dialog, Notification, nativeImage } = require('electron');
...
function loadNotificationSettings() {
  try {
    if (fs.existsSync(settingsPath)) {
      ...
      notificationSettings = { ...notificationSettings, ...saved };
    }
  } catch(e) {}
}
...
// ── 通知送信 ──
function sendNativeNotification({ title, body, icon, tabId }) {
  if (!Notification.isSupported()) return;
  const notif = new Notification({
    title,
    body,
    silent: false, // 音あり
    icon: icon ? nativeImage.createFromDataURL(icon) : nativeImage.createFromPath(path.join(__dirname, 'assets/icon.png')),
  });
  notif.on('click', () => {
    // 通知クリックでそのタブをアクティブに
    mainWindow.focus();
...
function getAppKeyFromUrl(url) {
  for (const [key, app] of Object.entries(notificationSettings.apps)) {
    if (url.includes(app.url)) return key;
  }
  return null;
}
...
ipcMain.handle('tab:create', async (event, url) => {
  ...
  view.webContents.on('page-title-updated', (e, t) => {
    ...
    if (!appKey || !notificationSettings.apps[appKey]?.enabled) return;

    // タイトルに未読数が含まれる場合に通知（例: "(3) Gmail" や "(1) Slack"）
    const match = t.match(/^\((\d+)\)\s*(.+)/);
    if (match) {
      const count = match[1];
      const pageTitle = match[2];
      const appLabel = notificationSettings.apps[appKey].label;
      sendNativeNotification({
        title: `${appLabel} - ${count}件の通知`,
        body: pageTitle,
        tabId: id,
      });
    }
  ...
  view.webContents.on('did-finish-load', () => {
    view.webContents.insertCSS(OVERLAY_CSS).catch(() => {});
  });

  // ── Web Notifications API をネイティブ通知に転送 ──
  view.webContents.on('ipc-message', () => {});
...
// ── Web Notifications をネイティブに転送するpreloadをwebviewに注入 ──
...
ipcMain.handle('notif:saveSettings', (e, settings) => {
  notificationSettings = settings;
  saveNotificationSettings();
  return true;
});
// ウェブページからのネイティブ通知転送
ipcMain.on('notif:send', (e, { title, body, icon, tabId }) => {
  if (!notificationSettings.enabled) return;
  // 送信元タブのURLチェック
  const tabEntry = [...webViews.entries()].find(([tid]) => tid === tabId);
  if (tabEntry) {
    const [, view] = tabEntry;
    const appKey = getAppKeyFromUrl(view._url || '');
    if (appKey && !notificationSettings.apps[appKey]?.enabled) return;
  }
  sendNativeNotification({ title, body, icon, tabId });
});
