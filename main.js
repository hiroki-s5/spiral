...
ipcMain.handle('tab:create', async (event, payload) => {
  ...
  const view = new BrowserView({
    webPreferences: {
      contextIsolation: true,
      nodeIntegration: false,
      backgroundThrottling: true,   // 非アクティブタブはスロットル→メモリ節約
      userAgent: UA,
...
function extractZipFromCrx(buf) {
  ...
  if (magic === 0x43723234) {
    ...
    // CRX2 with version field as header size (some variants)
    const pubKeyLen = buf.readUInt32LE(8);
    const sigLen = buf.readUInt32LE(12);
    return buf.slice(16 + pubKeyLen + sigLen);
  }

  // CRX3: magic = "CrX" = 0x43723458 (little-endian of "Cr48") 
  // actually "CrX3" bytes are 43 72 58 33 but stored as LE uint32
  if (magic === 0x43723458 || magic === 0x33587243) {
