...
function setupDrag(el, id, getTree, renderFn, dragType) {
  // ドラッグはハンドルを掴んだ時だけ有効にする（クリックとの競合を防ぐ）
  el.draggable = false;
...
function renderBMNode(node, container) {
  if (node.type === 'folder') {
    ...
    head.innerHTML = `
      ...
      <span class="folder-icon">${ARROW_SVG}</span>
    ...
    // ドラッグ設定
    setupDrag(head, node.id, () => S.bookmarks, renderBM, 'bm');
    setupDropTarget(head, node.id, () => S.bookmarks, renderBM, 'bm', true);
    ...
    // フォルダ自体への上下ドロップ
    setupDropTarget(wrap, node.id, () => S.bookmarks, renderBM, 'bm', false);
  } else {
    ...
    d.innerHTML = `<span class="drag-handle">${DRAG_SVG}</span>${ico}<span class="bm-name">${esc(node.name)}</span>`;
    ...
    setupDrag(d, node.id, () => S.bookmarks, renderBM, 'bm');
...
function renderTabNode(node, container) {
  if (node.type === 'folder') {
    ...
    head.innerHTML = `
      <span class="drag-handle">${DRAG_SVG}</span>
      <span class="folder-icon">${ARROW_SVG}</span>
      <span class="tab-title">${esc(node.name)}</span>
      <span style="font-size:10px;color:var(--textm);margin-right:4px">${count}</span>
    `;
    head.addEventListener('click', e => {
      if (e.target.closest('.drag-handle')) return;
    ...
    head.addEventListener('contextmenu', e => {
      ...
      showContextMenu(e.clientX, e.clientY, [
        ...
        { label: '削除（タブは残す）', action: () => {
          ...
          if (r) {
            const flat = flattenTabFolder(node);
            r.arr.splice(r.idx, 1, ...flat);
          }
          renderTabs(); scheduleSave();
        }},
      ]);
    });
    setupDrag(head, node.id, () => S.tabTree, renderTabs, 'tab');
    ...
    setupDropTarget(wrap, node.id, () => S.tabTree, renderTabs, 'tab', false);
  } else {
    // タブノード
    ...
    d.innerHTML = `<span class="drag-handle">${DRAG_SVG}</span>${ico}<span class="tab-title">${esc(t.title || 'New Tab')}</span>${canClose ? '<button class="tab-x">&#x2715;</button>' : ''}`;
    d.addEventListener('click', e => {
      if (e.target.closest('.drag-handle') || e.target.classList.contains('tab-x')) return;
      activateTab(t.id);
    });
    const xBtn = d.querySelector('.tab-x');
    if (xBtn) xBtn.addEventListener('click', e => closeTab(t.id, e));
    setupDrag(d, node.id, () => S.tabTree, renderTabs, 'tab');
...
document.getElementById('tab-folder-btn').addEventListener('click', () => {
  const name = prompt('フォルダ名を入力してください', '新しいフォルダ');
  if (!name) return;
  // アクティブなタブが選択されていたらそのタブをフォルダに入れる
  const newFolder = { id: 'tf_' + Date.now(), type: 'folder', name: name.slice(0,30), children: [], _open: true };
  if (S.active) {
    const r = findNode(S.tabTree, S.active);
    if (r) {
      const tabNode = { ...r.node };
      removeFromTree(S.tabTree, S.active);
      newFolder.children.push(tabNode);
    }
  }
  S.tabTree.push(newFolder);
  renderTabs(); scheduleSave();
});
