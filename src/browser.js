...
// ハンドル要素自体をdraggableにする（親要素はdraggableにしない）
...
function setupDropTarget(el, id, getTree, renderFn, dragType, isFolder) {
  ...
  el.addEventListener('drop', e => {
    ...
    const srcNode = JSON.parse(JSON.stringify(srcResult.node));
    if (isFolder) {
...
function renderBMNode(node, container) {
  if (node.type === 'folder') {
    ...
    dragH.innerHTML = DRAG_SVG;

    const iconSpan = document.createElement('span');
    iconSpan.className = 'folder-icon';
    iconSpan.innerHTML = ARROW_SVG;
    ...
    // head クリック → 開閉（ハンドル以外ならどこでも）
    head.addEventListener('click', e => {
      if (dragH === e.target || dragH.contains(e.target)) return;
    ...
    // ハンドル要素だけdraggable
    setupDrag(wrap, dragH, node.id, () => S.bookmarks, renderBM, 'bm');
    // head への上下ドロップ（フォルダ行の並び替え）
    ...
    icoEl.innerHTML = fav ? `<img class="bm-fav" src="${fav}" onerror="this.outerHTML='<div class=bm-fav-ph></div>'">` : '<div class="bm-fav-ph"></div>';
    ...
    d.appendChild(dragH);
    d.appendChild(icoEl.firstChild);
    ...
    setupDrag(d, dragH, node.id, () => S.bookmarks, renderBM, 'bm');
...
function renderTabNode(node, container) {
  if (node.type === 'folder') {
    ...
    const nameEl = document.createElement('span');
    nameEl.className = 'tab-title';
    nameEl.textContent = node.name;

    const cntEl = document.createElement('span');
    cntEl.style.cssText = 'font-size:10px;color:var(--textm);margin-right:4px';
    ...
    setupDrag(wrap, dragH, node.id, () => S.tabTree, renderTabs, 'tab');
    ...
    const dragH2 = document.createElement('span');
    dragH2.className = 'drag-handle';
    dragH2.innerHTML = DRAG_SVG;
    d.appendChild(dragH2);

    const icoEl2 = document.createElement('span');
    icoEl2.innerHTML = ico;
    d.appendChild(icoEl2.firstChild);
    ...
    d.addEventListener('click', e => {
      if (dragH2 === e.target || dragH2.contains(e.target)) return;
      if (e.target.classList.contains('tab-x')) return;
      activateTab(t.id);
    });
    setupDrag(d, dragH2, node.id, () => S.tabTree, renderTabs, 'tab');
