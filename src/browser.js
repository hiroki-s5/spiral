  ...
  S.pinnedApps.forEach((p, i) => {
    ...
    if (!p.isAdd) {
      const menuItems = [
        ...
        { label: '削除', action: () => { S.pinnedApps.splice(i, 1); renderPins(); scheduleSave(); } },
      ];
      d.addEventListener('contextmenu', e => {
        e.preventDefault();
...
// faviconが取れなかった場合のフォールバック（did-finish-load時にgoogle S2で補完）
if (window.browser.onFaviconFallback) {
  window.browser.onFaviconFallback(({ id, favicon }) => {
    const t = S.tabs.find(x => x.id === id);
    if (t && !t.fav) { t.fav = favicon; renderTabs(); }
  });
}
window.browser.onLoading(({ id, loading }) => {
  ...
  if (t) {
    t.loading = loading;
    // ロード完了後もfavがなければGoogle S2でフォールバック
    if (!loading && !t.fav && t.url && t.url.startsWith('http')) {
      try {
        const hostname = new URL(t.url).hostname;
        t.fav = 'https://www.google.com/s2/favicons?domain=' + hostname + '&sz=64';
      } catch {}
    }
    renderTabs();
  }
...
document.getElementById('import-go').addEventListener('click', async () => {
  ...
  // Cookie（ログイン状態）同期
  const cres = await window.browser.importCookies(selectedBrowser);
  if (cres.error) msgs.push('ログイン: ' + cres.error);
  else msgs.push(`✓ ログイン ${cres.count}件`);

  // パスワード自動同期（Keychain）
  const pres = await window.browser.importPasswordsAuto(selectedBrowser);
  if (pres.error) msgs.push('パスワード: ' + pres.error);
  else msgs.push(`✓ パスワード ${pres.count}件`);

  resEl.textContent = msgs.join('  ');
  resEl.classList.add('show');
  scheduleSave();
  setTimeout(() => importModal.classList.remove('show'), 2500);
...
window.browser.onNavigate(async ({ url }) => {
  ...
  matches.slice(0, 5).forEach(pw => {
    ...
    item.style.cssText = 'display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:8px;border:none;background:var(--hover);cursor:pointer;width:100%;text-align:left;';
    item.innerHTML = `<img src="https://www.google.com/s2/favicons?domain=${hostname}&sz=32" style="width:20px;height:20px;border-radius:4px" onerror="this.style.display='none'">
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:500;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${pw.user}</div>
  ...
  // 3秒後に自動で消える
  setTimeout(() => { autofillBanner.style.display = 'none'; }, 8000);
});
