...
document.getElementById('btn-history').addEventListener('click', () => {
  // Spiralの独自履歴（タブのナビゲーション履歴をブラウザバックで再現）
  if (S.active) window.browser.back(S.active);
  closeSB(0);
});

// ── フッター: ダウンロード（Electronのshell.openPathは使えないためGoogleドライブへ）
document.getElementById('btn-download').addEventListener('click', () => {
  newTab('https://drive.google.com/drive/my-drive');
  closeSB(0);
});
