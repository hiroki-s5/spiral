...
function openWorkspaceModal(idx) {
  ...
  document.getElementById('ws-email-input').value = ws.accountEmail || '';
  ...
  document.getElementById('ws-modal-confirm').onclick = () => {
    ...
    if (isNew) {
      S.workspaces.push({ id: 'ws' + Date.now(), name, avatar, color, accountEmail: email, tabs: [], bookmarks: [] });
    } else {
      S.workspaces[idx].name  = name;
      S.workspaces[idx].avatar = avatar;
      S.workspaces[idx].color = color;
      S.workspaces[idx].accountEmail = email;
      // 現在のワークスペースなら表示も更新
