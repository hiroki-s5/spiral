...
async function activateTab(id) {
  ...
  updateNav();
  // キーボードスクロールを適用
  if (S.keyboardScroll) window.browser.setKeyboardScroll(id, true).catch(() => {});
}
...
function openColorModal() {
  ...
  if (!modal) return;
  modal.style.display = 'flex';
  // 現在のテーマボタンのアクティブ状態を更新
  const lBtn = document.getElementById('theme-light-btn');
  const dBtn = document.getElementById('theme-dark-btn');
  if (lBtn) lBtn.style.borderColor = S.dark ? 'var(--panel-border)' : 'var(--accent)';
  if (dBtn) dBtn.style.borderColor = S.dark ? 'var(--accent)' : 'var(--panel-border)';
  // カスタムカラーピッカーの初期値
  const curAccent = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim();
  const ac = document.getElementById('accent-custom');
  if (ac && curAccent.startsWith('#')) ac.value = curAccent;
  const curPanel = getComputedStyle(document.documentElement).getPropertyValue('--panel').trim();
  const pc = document.getElementById('panel-custom');
  if (pc && curPanel.startsWith('#')) pc.value = curPanel;
}
...
function setThemeMode(mode) {
  ...
  const hm = document.getElementById('h-ico-m');
  const hs = document.getElementById('h-ico-s');
  if (hm) hm.style.display = S.dark ? 'none' : '';
  if (hs) hs.style.display = S.dark ? '' : 'none';
...
function applyAccentColor(color) {
  ...
  document.documentElement.style.setProperty('--accent-s', `rgba(${r},${g},${b},0.15)`);
  document.documentElement.style.setProperty('--spin', color);
  S.accentColor = color;
  scheduleSave();
}

function applyPanelColor(color) {
  document.documentElement.style.setProperty('--panel', color);
  S.panelColor = color;
  scheduleSave();
}
...
function resetThemeColors() {
  ...
  document.documentElement.style.removeProperty('--spin');
...
document.getElementById('nb-theme').addEventListener('click', () => openColorModal());
...
window.browser.onUpdateAvailable(({ version }) => {
  ...
  if (!bar) return;
  // 水平バーのアップデートボタンを表示
  const hBtnUpdate = document.getElementById('h-btn-update');
  if (hBtnUpdate) hBtnUpdate.style.display = '';
...
function openSettingsModal() {
  clearTimeout(hideT);
  hideT = null;
  _pendingLayout = S.layout;
  document.getElementById('settings-modal').style.display = 'flex';
  // 専用IPCで設定オープンをmain processに伝える
  try { window.browser.send('ui:modal-state', true); } catch {}
  _lastModalOpenState = true;
  switchSettingsPane('general');
}
function closeSettingsModal() {
  document.getElementById('settings-modal').style.display = 'none';
  _lastModalOpenState = false;
  // 設定を閉じたらmain processに通知してUIを復元
  try { window.browser.send('ui:modal-state', false); } catch {}
  // 少し遅延してsb:openを送り、UIを確実に展開
  setTimeout(() => {
    if (!_sbOpen) {
      _sbOpen = true;
      sb.classList.add('open');
...
function switchSettingsPane(pane) {
  ...
  if (pane === 'mail') mailRenderAddressList();
  document.getElementById('settings-pane-title').textContent =
    pane === 'general' ? '一般' : pane === 'network' ? 'ネットワーク' : pane === 'download' ? 'ダウンロード' : 'メールアドレス';
...
function initHorizontalUI() {
  ...
  // ナビボタン（アドレス行）
  bind('h-back',   () => { if (S.active) window.browser.back(S.active); });
  bind('h-fwd',    () => { if (S.active) window.browser.forward(S.active); });
  ...
  // ダークモード
  bind('h-theme', () => openColorModal());
  bind('h-btn-update', () => document.getElementById('btn-update')?.click());
  ...
  // h-btn-notif: btn-notifのmousedownを直接発火
  const hBtnNotif = document.getElementById('h-btn-notif');
  if (hBtnNotif && !hBtnNotif._bound) {
    hBtnNotif._bound = true;
    hBtnNotif.addEventListener('mousedown', e => {
      document.getElementById('btn-notif').dispatchEvent(new MouseEvent('mousedown', { bubbles: true }));
    });
    hBtnNotif.addEventListener('mouseup', e => {
      document.getElementById('btn-notif').dispatchEvent(new MouseEvent('mouseup', { bubbles: true }));
    });
    hBtnNotif.addEventListener('click', e => {
      document.getElementById('btn-notif').dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
  }
  bind('h-btn-ai',       () => document.getElementById('btn-ai').click());
  bind('h-btn-settings', () => { window.browser.openSettings(); });
...
function saveSettingToggle(key, value) {
  S[key] = value;
  scheduleSave();
  if (key === 'keyboardScroll') applyKeyboardScroll();
  if (key === 'dblclickClose') renderTabs();
}

function applyKeyboardScroll() {
  // アクティブなWebViewにキーボードスクロールを有効/無効にするJSを注入
  if (S.active) window.browser.setKeyboardScroll(S.active, S.keyboardScroll).catch(() => {});
}
...
function saveAppState() {
  ...
  const state = {
    ...
    dblclickClose: S.dblclickClose || false,
    keyboardScroll: S.keyboardScroll || false,
    accentColor: S.accentColor || null,
    panelColor: S.panelColor || null,
  };
...
// 設定ウィンドウで保存が押されたときにmain windowに反映
window.browser.onSettingsApplied && window.browser.onSettingsApplied(state => {
  if (!state) return;
  if (state.layout && state.layout !== S.layout) {
    S.layout = state.layout;
    applyLayout(S.layout);
    window.browser.setLayout(S.layout);
  }
  if (state.dark !== undefined && state.dark !== S.dark) {
    S.dark = state.dark;
    document.documentElement.setAttribute('data-theme', S.dark ? 'dark' : 'light');
    document.getElementById('ico-m').style.display = S.dark ? 'none' : '';
    document.getElementById('ico-s').style.display = S.dark ? '' : 'none';
  ...
  if (state.dblclickClose !== undefined) {
    S.dblclickClose = state.dblclickClose;
    renderTabs(); // タブを再描画してdblclickイベントを再登録
  }
  if (state.keyboardScroll !== undefined) {
    S.keyboardScroll = state.keyboardScroll;
    if (S.active) window.browser.setKeyboardScroll(S.active, S.keyboardScroll).catch(() => {});
  }
...
window.browser.onReady(async () => {
  ...
  if (saved) {
    ...
    if (saved.dblclickClose !== undefined) {
      S.dblclickClose = saved.dblclickClose;
      const _dcc = document.getElementById('setting-dblclick-close'); if (_dcc) _dcc.checked = S.dblclickClose;
    }
    if (saved.keyboardScroll !== undefined) {
      S.keyboardScroll = saved.keyboardScroll;
      const _ksc = document.getElementById('setting-keyboard-scroll'); if (_ksc) _ksc.checked = S.keyboardScroll;
    }
    if (saved.accentColor) {
      S.accentColor = saved.accentColor;
      applyAccentColor(saved.accentColor);
    }
    if (saved.panelColor) {
      S.panelColor = saved.panelColor;
      document.documentElement.style.setProperty('--panel', saved.panelColor);
    }
...
(function() {
  const modalIds = [
    ...
    'notif-hub-panel', 'prompt-modal', 'split-modal', 'ext-panel',
    // settings-modal は openSettingsModal/closeSettingsModal で直接管理するため除外
  ];
...
async function runSecurityDiagnosis() {
  const resultEl = document.getElementById('security-diag-result');
  if (!resultEl) return;
  const tabId = S.active;
  if (!tabId) {
    resultEl.style.display = 'block';
    resultEl.textContent = 'アクティブなタブがありません。';
    return;
  }
  resultEl.style.display = 'block';
  resultEl.textContent = '診断中...';
  try {
    const diag = await window.browser.diagnoseTab(tabId);
    if (!diag) { resultEl.textContent = '診断できませんでした。'; return; }
    const lines = [];
    lines.push(diag.isHttps ? 'HTTPS接続（安全）' : 'HTTP接続（暗号化なし）');
    lines.push(diag.suspicious ? '不審なURLパターンを検出' : 'URLは正常');
    lines.push(`ドメイン: ${diag.hostname || '不明'}`);
    resultEl.innerHTML = lines.join('<br>');
    resultEl.style.color = diag.suspicious ? '#ff4757' : 'var(--text2)';
  } catch(e) {
...
// ═══════════════════════════════════════════
//  捨てメールアドレス機能（1secmail API）
// ═══════════════════════════════════════════

// 発行済みアドレスをlocalStorageで管理
function mailLoadAddresses() {
  try { return JSON.parse(localStorage.getItem('spiral_mail_addresses') || '[]'); } catch { return []; }
}
function mailSaveAddresses(list) {
  localStorage.setItem('spiral_mail_addresses', JSON.stringify(list));
}

// ランダムなユーザー名を生成
function mailRandomUsername() {
...
// 発行済みアドレス一覧をレンダリング
function mailRenderAddressList() {
  const list = mailLoadAddresses();
  const container = document.getElementById('mail-address-list');
  const inboxSelect = document.getElementById('mail-inbox-select');
  if (!container) return;

  container.innerHTML = '';
  // inboxSelectを更新
  if (inboxSelect) {
    inboxSelect.innerHTML = '<option value="">アドレスを選択</option>';
    list.forEach(addr => {
  ...
  if (list.length === 0) {
    container.innerHTML = '<div style="font-size:12px;color:var(--text2);">まだアドレスがありません。上のフォームから発行してください。</div>';
    return;
  }

  list.forEach(addr => {
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:10px;border:1px solid var(--panel-border);background:var(--card);';
    row.innerHTML = `
      <div style="flex:1;font-size:12px;font-family:monospace;color:var(--text);word-break:break-all;">${addr}</div>
      <button onclick="mailCopyAddress('${addr}')" style="padding:5px 10px;border-radius:7px;border:none;background:var(--accent);color:#fff;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;white-space:nowrap;">コピー</button>
      <button onclick="mailDeleteAddress('${addr}')" style="padding:5px 10px;border-radius:7px;border:1px solid var(--panel-border);background:var(--card);color:var(--text2);font-size:11px;font-weight:500;cursor:pointer;font-family:inherit;white-space:nowrap;">削除</button>
    `;
...
function mailGenerate() {
  const userInput = document.getElementById('mail-username-input');
  const domainSelect = document.getElementById('mail-domain-select');
  if (!userInput || !domainSelect) return;

  let username = userInput.value.trim().toLowerCase().replace(/[^a-z0-9._-]/g, '');
  if (!username) { userInput.focus(); userInput.style.borderColor = 'var(--accent)'; return; }
  ...
  list.push(addr);
  mailSaveAddresses(list);
  userInput.value = '';
  mailRenderAddressList();
}

// ランダム発行
function mailGenerateRandom() {
  const domainSelect = document.getElementById('mail-domain-select');
  const userInput = document.getElementById('mail-username-input');
  if (!domainSelect) return;
  const username = mailRandomUsername();
  const domain = domainSelect.value;
  const addr = username + '@' + domain;
  const list = mailLoadAddresses();
  list.push(addr);
...
// アドレスをコピー
function mailCopyAddress(addr) {
  navigator.clipboard.writeText(addr).catch(() => {});
}

// アドレスを削除
function mailDeleteAddress(addr) {
  const list = mailLoadAddresses().filter(a => a !== addr);
  mailSaveAddresses(list);
  mailRenderAddressList();
}

// 受信トレイを取得（1secmail API）
async function mailFetchInbox() {
  const select = document.getElementById('mail-inbox-select');
  const inboxList = document.getElementById('mail-inbox-list');
  if (!select || !inboxList) return;

  const addr = select.value;
  if (!addr) {
    inboxList.innerHTML = '<div style="color:var(--text2);font-size:12px;text-align:center;padding:24px 0;">アドレスを選択してください</div>';
    return;
  }

  const [login, domain] = addr.split('@');
  inboxList.innerHTML = '<div style="color:var(--text2);font-size:12px;text-align:center;padding:24px 0;">取得中...</div>';
  ...
  try {
    ...
    if (!Array.isArray(messages) || messages.length === 0) {
      inboxList.innerHTML = '<div style="color:var(--text2);font-size:12px;text-align:center;padding:24px 0;">メールはありません</div>';
      return;
    }
    ...
    messages.forEach(msg => {
      const row = document.createElement('div');
      row.style.cssText = 'padding:10px 12px;border-radius:10px;border:1px solid var(--panel-border);background:var(--card);cursor:pointer;transition:background .1s;';
      row.onmouseover = () => row.style.background = 'var(--hover)';
      row.onmouseout  = () => row.style.background = 'var(--card)';
      const date = new Date(msg.date).toLocaleString('ja-JP', { month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit' });
      row.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
          <div style="font-size:12px;font-weight:600;color:var(--text);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escHtml(msg.subject || '(件名なし)')}</div>
          <div style="font-size:11px;color:var(--text2);flex-shrink:0;margin-left:8px;">${date}</div>
        </div>
        <div style="font-size:11px;color:var(--text2);">From: ${escHtml(msg.from || '')}</div>
      `;
      row.addEventListener('click', () => mailOpenMessage(login, domain, msg.id, msg.subject, msg.from));
      inboxList.appendChild(row);
    });
  } catch(e) {
    inboxList.innerHTML = `<div style="color:#ff4757;font-size:12px;text-align:center;padding:24px 0;">取得失敗: ${escHtml(e.message)}</div>`;
  }
}

// メール本文を取得して表示
async function mailOpenMessage(login, domain, id, subject, from) {
  const bodyArea = document.getElementById('mail-body-area');
  const bodySubject = document.getElementById('mail-body-subject');
  const bodyFrom = document.getElementById('mail-body-from');
  const bodyContent = document.getElementById('mail-body-content');
  if (!bodyArea) return;

  bodySubject.textContent = subject || '(件名なし)';
  bodyFrom.textContent = 'From: ' + (from || '');
  bodyContent.textContent = '読み込み中...';
  ...
  try {
    ...
    // HTMLメールはテキスト部分を優先
    bodyContent.textContent = msg.textBody || msg.htmlBody?.replace(/<[^>]+>/g, '') || '(本文なし)';
  } catch(e) {
    bodyContent.textContent = '取得失敗: ' + e.message;
  }
}
...
let _smsSelectedNumber = '';

// 番号一覧を取得（複数サービスにフォールバック）
async function smsLoadNumbers() {
  const countrySelect = document.getElementById('sms-country-select');
  const numberList = document.getElementById('sms-number-list');
  if (!countrySelect || !numberList) return;

  const country = countrySelect.value;
  numberList.innerHTML = '<div style="color:var(--text2);font-size:12px;text-align:center;padding:16px 0;">取得中...</div>';

  try {
    const result = await window.browser.getPhoneNumbers(country);
    if (result.ok && result.numbers.length > 0) {
      smsRenderNumbers(result.numbers, numberList);
    } else {
      numberList.innerHTML = '<div style="color:var(--text2);font-size:12px;text-align:center;padding:16px 0;">番号を取得できませんでした。しばらくしてから再試行してください。</div>';
    }
  } catch(e) {
    numberList.innerHTML = '<div style="color:#ff4757;font-size:12px;text-align:center;padding:16px 0;">取得失敗: ' + escHtml(e.message) + '</div>';
  }
}


// 番号一覧をレンダリング
function smsRenderNumbers(numbers, numberList) {
  numberList.innerHTML = '';
  numbers.forEach(num => {
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:9px;border:1px solid var(--panel-border);background:var(--card);';
    row.innerHTML = `
      <div style="flex:1;font-size:13px;font-family:monospace;color:var(--text);letter-spacing:.03em;">${escHtml(num)}</div>
      <button onclick="smsSelectNumber('${escHtml(num)}')" style="padding:6px 12px;border-radius:7px;border:none;background:var(--accent);color:#fff;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;white-space:nowrap;">この番号を使う</button>
    `;
...
// 番号を選択
function smsSelectNumber(num) {
  _smsSelectedNumber = num;
  const inboxArea = document.getElementById('sms-inbox-area');
  const selectedNum = document.getElementById('sms-selected-number');
  if (inboxArea) inboxArea.style.display = 'block';
  if (selectedNum) selectedNum.textContent = num;
  const msgList = document.getElementById('sms-message-list');
  if (msgList) msgList.innerHTML = '<div style="color:var(--text2);font-size:12px;text-align:center;padding:16px 0;">「SMS更新」を押して受信確認</div>';
}

// 選択中の番号をコピー
function smsCopyNumber() {
  if (_smsSelectedNumber) navigator.clipboard.writeText(_smsSelectedNumber).catch(() => {});
}

// SMSを取得
async function smsFetchMessages() {
  const msgList = document.getElementById('sms-message-list');
  if (!msgList || !_smsSelectedNumber) return;

  msgList.innerHTML = '<div style="color:var(--text2);font-size:12px;text-align:center;padding:16px 0;">取得中...</div>';

  try {
    const result = await window.browser.getPhoneSMS(_smsSelectedNumber);
    if (result.ok && result.messages.length > 0) {
      smsFetchRender(result.messages, msgList);
    } else {
      msgList.innerHTML = '<div style="color:var(--text2);font-size:12px;text-align:center;padding:16px 0;">SMSはありません。この番号宛にSMSを送ってから更新してください。</div>';
    }
  } catch(e) {
    msgList.innerHTML = '<div style="color:#ff4757;font-size:12px;text-align:center;padding:16px 0;">取得失敗: ' + escHtml(e.message) + '</div>';
  }
}

function smsFetchRender(messages, msgList) {
  msgList.innerHTML = '';
  messages.slice(0, 20).forEach(msg => {
    const row = document.createElement('div');
    row.style.cssText = 'padding:11px 13px;border-radius:9px;border:1px solid var(--panel-border);background:var(--card);';
    row.innerHTML = `
      <div style="display:flex;justify-content:space-between;margin-bottom:5px;">
        <div style="font-size:11px;color:var(--text2);">${msg.from ? 'From: ' + escHtml(msg.from) : ''}</div>
        <div style="font-size:11px;color:var(--text2);">${escHtml(msg.time)}</div>
      </div>
      <div style="font-size:13px;color:var(--text);line-height:1.6;word-break:break-all;">${escHtml(msg.text)}</div>
    `;
