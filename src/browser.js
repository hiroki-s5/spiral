...
async function newTab(url) {
  ...
  const id = await window.browser.createTab(targetUrl, wsId);
  S.tabs.push({ id, title: '読み込み中...', url: targetUrl, fav: null, loading: true });
  ...
  exts.forEach(ext => {
    ...
    d.innerHTML = `
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:11px;color:var(--text2);">v${esc(ext.version)}</div>
      </div>
    `;
    list.appendChild(d);
  });
...
document.addEventListener('keydown', e => {
  ...
  // ズーム
  if (m && (e.key === '+' || e.key === '=' || e.key === ';')) { e.preventDefault(); if (S.active) window.browser.zoomIn(S.active).then(pct => showZoomIndicator(pct)); }
  if (m && e.key === '-') { e.preventDefault(); if (S.active) window.browser.zoomOut(S.active).then(pct => showZoomIndicator(pct)); }
  if (m && e.key === '0') { e.preventDefault(); if (S.active) window.browser.zoomReset(S.active).then(pct => showZoomIndicator(pct)); }
});
...
function showZoomIndicator(pct) {
  ...
  zoomIndicatorTimer = setTimeout(() => { el.style.opacity = '0'; }, 1200);
}
...
function switchWorkspace(idx) {
  ...
  // ワークスペース0以外でも直接タブ切り替え（ログイン強制を廃止）
...
function openWorkspaceModal(idx) {
  ...
  // 既存リスナーをcloneNodeで削除してから新しいのを登録（重複防止）
