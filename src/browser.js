...
if (window.browser.onExtInstalled) {
  window.browser.onExtInstalled(({ extId }) => {
    showNotifToast('拡張機能をインストールしました');
  });
}
...
function showWsLoginModal(wsIdx) {
  ...
  modal.innerHTML = `
    ...
    <button id="ws-login-skip-btn" style="background:transparent;border:none;color:var(--text2);font-size:12px;cursor:pointer;font-family:inherit;padding:4px;">
      ログインせずに使う
    </button>
  `;
  modal.style.display = 'flex';
  ...
  document.getElementById('ws-login-skip-btn').addEventListener('click', () => {
    ...
    hideWsLoginModal();
    // タブがなければ新しいタブを作る
    if (S.tabs.length === 0) newTab('https://www.google.com');
    scheduleSave();
  });

  document.getElementById('ws-login-go-btn').addEventListener('click', async () => {
    hideWsLoginModal();
    // 既存タブを閉じずに新しいタブでログインページを開く
