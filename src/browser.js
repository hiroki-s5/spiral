...
// 拡張機能インストール完了通知
if (window.browser.onExtInstalled) {
  window.browser.onExtInstalled(({ extId }) => {
    showNotifToast('拡張機能をインストールしました（再起動で有効になります）');
    renderExtList();
  });
}
...
async function renderExtList() {
  ...
  if (!list) return;
  const exts = await window.browser.getExtensions();
  if (!exts || !exts.length) {
    list.innerHTML = '<div style="font-size:12px;color:var(--text2);padding:8px 0;">インストール済みの拡張機能はありません</div>';
    return;
  }
  list.innerHTML = '';
  exts.forEach(ext => {
    const d = document.createElement('div');
    d.style.cssText = 'display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--card-border);';
    d.innerHTML = `
      <div style="flex:1;min-width:0;">
        <div style="font-size:12px;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(ext.name)}</div>
        <div style="font-size:11px;color:var(--textm);">v${esc(ext.version)}</div>
      </div>
      <button class="ext-run-btn" data-extid="${esc(ext.id)}" style="background:var(--accent);border:none;color:#fff;border-radius:7px;padding:3px 10px;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;white-space:nowrap;">使う</button>
      <button class="ext-del-btn" data-extid="${esc(ext.id)}" data-extname="${esc(ext.name)}" title="アンインストール" style="background:transparent;border:1px solid var(--card-border);color:var(--textm);border-radius:7px;padding:3px 8px;font-size:11px;cursor:pointer;font-family:inherit;white-space:nowrap;transition:background 0.1s,color 0.1s;">削除</button>
    `;
    list.appendChild(d);
  });

  // 「使う」ボタン → 拡張機能のpopupを開く
  list.querySelectorAll('.ext-run-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      const extId = btn.dataset.extid;
      document.getElementById('ext-modal').style.display = 'none';
      await window.browser.openExtensionPopup(extId);
    });
  });

  // 「削除」ボタン → アンインストール確認
  list.querySelectorAll('.ext-del-btn').forEach(btn => {
    btn.addEventListener('mouseenter', () => { btn.style.background = 'rgba(244,67,54,0.12)'; btn.style.color = '#f44336'; btn.style.borderColor = '#f44336'; });
    btn.addEventListener('mouseleave', () => { btn.style.background = 'transparent'; btn.style.color = 'var(--textm)'; btn.style.borderColor = 'var(--card-border)'; });
    btn.addEventListener('click', async () => {
      const extId = btn.dataset.extid;
      ...
      btn.textContent = '確認中...';
      btn.disabled = true;
      const confirmed = await showPrompt(`「${extName}」を削除しますか？\n削除するには「削除」と入力してください`);
      if (confirmed !== '削除') {
        btn.textContent = '削除';
        btn.disabled = false;
        return;
      ...
      btn.textContent = '削除中...';
      const result = await window.browser.uninstallExtension(extId);
      if (result && result.error) {
        btn.textContent = 'エラー';
        btn.style.color = '#f44336';
        setTimeout(() => { btn.textContent = '削除'; btn.disabled = false; btn.style.color = 'var(--textm)'; }, 2000);
      } else {
        showNotifToast(`「${extName}」を削除しました`);
        await renderExtList();
      }
    });
  });
...
function openExtModal() {
  ...
  modal.style.display = 'flex';
  document.getElementById('ext-id-input').value = '';
  document.getElementById('ext-status').textContent = '';
  renderExtList();
  openSB();
  setTimeout(() => document.getElementById('ext-id-input').focus(), 40);
}

document.getElementById('btn-extensions').addEventListener('click', openExtModal);
document.getElementById('ext-modal-cancel').addEventListener('click', () => {
  document.getElementById('ext-modal').style.display = 'none';
});
document.getElementById('ext-modal').addEventListener('click', e => {
  if (e.target === document.getElementById('ext-modal')) document.getElementById('ext-modal').style.display = 'none';
});
...
document.getElementById('ext-install-btn').addEventListener('click', async () => {
  ...
  // URLから抽出またはそのまま使用
  let extId = input.value.trim();
  const m = extId.match(/([a-z]{32})/);
  if (m) extId = m[1];
  if (!extId || extId.length !== 32) {
    status.style.color = '#f44336';
    status.textContent = '有効な拡張機能IDを入力してください（32文字）';
    return;
  }
  status.style.color = 'var(--text2)';
  status.textContent = 'インストール中...';
  document.getElementById('ext-install-btn').disabled = true;
  const result = await window.browser.installExtension(extId);
  document.getElementById('ext-install-btn').disabled = false;
  if (result && result.error) {
    status.style.color = '#f44336';
...
document.getElementById('ext-id-input').addEventListener('keydown', e => {
  if (e.key === 'Enter') document.getElementById('ext-install-btn').click();
});
...
function renderNotifModal() {
  ...
  Object.entries(notifSettings.apps).forEach(([key, app]) => {
    ...
    const iconSrc = APP_ICONS[key] || `https://www.google.com/s2/favicons?domain=${app.url}&sz=64`;
