...
// Chrome Web Store: Spiralに追加ボタン用のAPIをWindowに公開
window.browser = window.browser || {};
window.browser.installExtFromStore = function(extId) {
  return ipcRenderer.invoke('ext:install-from-store', extId);
};

// 拡張機能のchrome APIスタブ（Electronでサポートされない機能をスタブ化）
try {
  if (typeof chrome !== 'undefined') {
    ...
    if (!chrome.notifications) {
      chrome.notifications = {
        create: function(id, opts, cb) { cb && cb(id || 'n1'); },
        clear: function(id, cb) { cb && cb(true); },
    ...
    if (!chrome.contextMenus) {
      chrome.contextMenus = {
        create: function() { return 'cm1'; },
        update: function(id, props, cb) { cb && cb(); },
        remove: function(id, cb) { cb && cb(); },
        removeAll: function(cb) { cb && cb(); },
        onClicked: { addListener: function() {} },
      };
    }
    // tabs.onActivated等の追加stub
    if (chrome.tabs && !chrome.tabs.onActivated) {
      chrome.tabs.onActivated = { addListener: function() {}, removeListener: function() {} };
    }
    // webRequest stub
    if (!chrome.webRequest) {
      chrome.webRequest = {
        onBeforeRequest:  { addListener: function() {}, removeListener: function() {} },
        onBeforeSendHeaders: { addListener: function() {}, removeListener: function() {} },
        onHeadersReceived: { addListener: function() {}, removeListener: function() {} },
